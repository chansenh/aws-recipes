...
def processRecipes(txtfile):
    ...
    for recipe in recipelist:
        print("=================================",recipe["name"])
        configstring="""{{\n"name":{}\n"image":{}\n"category":[{}]\n"ingredients":[{}]\n"instructions":{}\n}}"""
        output = configstring.format(recipe["name"],recipe["image"],recipe["categories"],recipe["ingredients"],recipe["instructions"])
        print(output)
        datastore.append(output)

    
    newjson = json.dumps(data)
    datafile = open("datascheme.txt", "w")
    datafile.write("\n".join(datastore))
    datafile.close()
